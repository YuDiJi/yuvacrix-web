...
const TeamSelection = ({ teamA, teamB }: Props) => {
  ...
  return (
    <div className="flex min-h-full flex-col bg-(--color-bg-base)">
      <div className="flex flex-1 flex-col gap-5 p-4">
          ...
          >
            ...
            >
              <Users
                size={22}
                className={
                  teamA ? "text-white/70" : "text-(--color-text-muted)"
                }
              />
            </div>
          ...
          >
            ...
            >
              <Users
                size={22}
                className={
                  teamB ? "text-white/70" : "text-(--color-text-muted)"
                }
              />
            </div>
            <div className="text-center">
        ...
        <div className="relative flex flex-col gap-3">
          ...
          <button
            onClick={() => router.push("/start-match/select-team?team=A")}
            className={cn(
              "relative flex flex-col items-center justify-center gap-3 rounded-2xl border-2 p-8 overflow-hidden",
              "transition-all duration-200 active:scale-[0.98]",
              teamA
                ? "border-(--color-brand) bg-(--color-navy) shadow-[0_4px_20px_rgba(27,63,160,0.20)]"
                : "border-(--color-bg-border) bg-(--color-bg-card) hover:border-(--color-sky)/40",
          ...
          >
            {/* Logo background — only when team selected and has a logo */}
            {teamA?.logoUrl && (
              <>
                {/* Absolutely-positioned wrapper so S3Image (width/height) fills the card */}
                ...
                <span className="absolute inset-0 z-0 bg-linear-to-t from-(--color-navy)/50 via-(--color-navy)/20 to-(--color-navy)/10" />
              </>
            )}

            {/* Check badge */}
            {teamA && (
              <span className="absolute right-4 top-4 z-10 flex h-6 w-6 items-center justify-center rounded-full bg-(--color-brand)">
                <Check size={12} className="text-white" strokeWidth={3} />
              </span>
            )}
            ...
            <div
              className={cn(
                "relative z-10 flex h-12 w-12 items-center justify-center rounded-full border-2 overflow-hidden transition-all",
                teamA
                  ? "border-white/20 bg-white/10"
                  : "border-(--color-bg-border) bg-(--color-bg-base)",
              )}
            ...
            {!teamA && (
              <span className="relative z-10 flex items-center gap-1 rounded-full border border-(--color-bg-border) bg-(--color-bg-base) px-3 py-1 text-xs font-semibold text-(--color-text-muted)">
                Tap to select <ChevronRight size={11} />
              </span>
            )}
          </button>
      ...
      {/* Continue CTA */}
      <div className="border-t border-(--color-bg-border) bg-(--color-bg-base) p-4">
        <button
          disabled
