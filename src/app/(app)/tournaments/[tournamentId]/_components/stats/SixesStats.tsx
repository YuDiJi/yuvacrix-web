// _components/stats/SixesStats.tsx
...
export default function SixesStats({ tournamentId, teamId }: Props) {
  const { data, isLoading, isFetching, isError, refetch } =
    useGetTournamentBoundaryStatsQuery({
      tournamentId,
      query: {
        type: TournamentBoundaryStatType.SIXES,
        teamId,
        roundId: "ALL",
        groupId: "ALL",
        skip: 0,
  ...
  if (isError || !data) {
    return (
      <BoundaryMessage
        type="error"
        title="Unable to load sixes"
        description="Something went wrong while loading the sixes leaderboard."
  ...
  if (data.summary.includedCompletedMatches === 0 || !data.items.length) {
    return (
      <BoundaryMessage
        title="No sixes recorded yet"
        description="Sixes stats will appear after completed tournament matches."
      />
    );
  }
  ...
  return (
    <div className="relative pb-24">
      ...
      {isFetching && <RefreshingIndicator label="Updating Sixes" />}
    </div>
  );
}
